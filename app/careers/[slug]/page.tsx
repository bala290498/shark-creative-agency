...
interface CareersPostPageProps {
  params: {
    slug: string
  }
}
...
export async function generateStaticParams() {
  try {
    ...
    const params = positions.map((position) => ({
      slug: position.slug,
    }))
    console.log('Generated static params for career positions:', params.map(p => p.slug))
    return params
  } catch (error) {
...
export async function generateMetadata({ params }: CareersPostPageProps): Promise<Metadata> {
  ...
  return {
    title: `${position.title} | Shark Creative Agency Careers`,
...
export default function CareersPost({ params }: CareersPostPageProps) {
  const slug = params.slug
  const position = getPositionBySlug(slug)

  if (!position) {
    console.error(`Career position not found for slug: ${slug}`)
    notFound()
  }
